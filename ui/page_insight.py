...
플랫폼 운영자(관리자)가 "어떤 사용자가 이탈 위험이 높은지" 한눈에 보는 분석 화면입니다.
  1) 핵심 숫자 4개 (분석 사용자 수, 이탈률 ...)
  2) 핵심 신호 — 이탈이 많은 그룹과 적은 그룹의 이탈률 비교
  3) 자기소개는 '개수'뿐 아니라 '길이'도 중요하다는 것 (색으로 보는 표) + 프로필 완성도 5구간
  4) 사용자들이 자주 비워두는 항목과, 빈칸을 어떻게 다뤘는지
  5) 이런 사용자가 위험해요 — 분석 결과를 조합해 만든 '가상 사용자' 카드 4장 (얼굴 포함)
  6) 위험군 분포(Low/Medium/High)와 모델이 중요하게 본 항목(SHAP 순위)
  7) 연령대 / 관계 상태 / 자기소개 작성량 / 라이프스타일별 이탈률 (데이터 파일이 있을 때)

[숫자는 어디서 오나요?]
- 1~6번: project_facts.py 에 적어 둔 값.
         팀 노트북(02_essay_deep_experiment)의 Train 데이터(44,959명) EDA 결과와 노션 결과서 기준이에요.
- 7번  : data 폴더의 데이터 파일을 읽어서 직접 계산 (insight_data.py).
    ...
    with c2:
        show(check_list_card("빈칸은 이렇게 다뤘어요", "지우거나 평균값으로 채우지 않고 그대로 살렸어요.", [
            ...
            ("자녀 항목", "무응답 여부를 따로 표시(has_kids_na)해요. 무응답이면 이탈률이 "
                      f"{facts.KEY_SIGNALS[2]['high_rate']:.1f}%, 응답하면 {facts.KEY_SIGNALS[2]['low_rate']:.1f}%예요."),
            ("소득", "비공개(-1)를 실제 소득처럼 계산하지 않도록 빈칸으로 바꿔요."),
        ]))

    # ── 5-2) 이런 사용자가 위험해요: 가상 사용자(페르소나) 카드 ──
